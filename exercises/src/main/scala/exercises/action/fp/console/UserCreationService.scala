...
class UserCreationService(console: Console, clock: Clock) {
  ...
  val readDateOfBirth: IO[LocalDate] = {
    ...
    dateIO.onError(_=> writeLine("""Incorrect format, for example enter "18-03-2001" for 18th of March 2001"""))
  }

  // 3. Refactor `readSubscribeToMailingList` and `readUser` using the same techniques as `readDateOfBirth`.
  val readSubscribeToMailingList: IO[Boolean] = (for {
    _ <- writeLine("Would you like to subscribe to our mailing list? [Y/N]")
    line <- readLine
    yesNo <- parseLineToBoolean(line)
  } yield yesNo).onError(_=>writeLine("""Incorrect format, enter "Y" for Yes or "N" for "No""""))

  val readUser: IO[User] = for {
    name <- readName
    dateOfBirth <- readDateOfBirth.retry(3)
    yesNo <- readSubscribeToMailingList.retry(3)
    createdAt <- clock.now
    user = User(name, dateOfBirth, yesNo, createdAt)
    _ <- writeLine(s"User is $user").map(_ => user)
  } yield user
