...
object SearchResult {
  private def distinct(flights: List[Flight]): List[Flight] = {
    flights.groupBy(_.flightId).map(tuple => tuple._2.minBy(_.unitPrice)).toList
  }

  def apply(flights: List[Flight]): SearchResult = {
    val sorted = distinct(flights).sorted(bestOrdering)
