...
object SearchFlightService {
  implicit val flightOrdering: Ordering[Flight] = SearchResult.bestOrdering
  implicit val globalEx: ExecutionContext = ExecutionContext.global

  private def searchByClient(from: Airport, to: Airport, date: LocalDate)(client: SearchFlightClient): IO[List[Flight]] = {
    client.search(from, to, date).handleErrorWith(ex => IO.debug(ex.getMessage).andThen(IO(List.empty[Flight])))
  }
  ...
  def fromTwoClients(client1: SearchFlightClient, client2: SearchFlightClient): SearchFlightService = {
    ...
    new SearchFlightService {
      def search(from: Airport, to: Airport, date: LocalDate): IO[SearchResult] = {
        searchByClient(from, to, date)(client1)
          .parZip(searchByClient(from, to, date)(client2))(globalEx)
          .map(ls => SearchResult(ls._1 ++ ls._2))

        /*for {
          fl1 <- searchByClient(from,to,date)(client1)
          fl2 <- searchByClient(from,to,date)(client2)
        } yield SearchResult(fl1++fl2)*/
      }
    }
  }
  ...
  def fromClients(clients: List[SearchFlightClient]): SearchFlightService =
    new SearchFlightService {
      ...
      def search(from: Airport, to: Airport, date: LocalDate): IO[SearchResult] = {
        val value = IO.parTraverse(clients)(client => searchByClient(from, to, date)(client))(globalEx)
          .map(_.flatten)
          .map(SearchResult(_))
        value
      }
