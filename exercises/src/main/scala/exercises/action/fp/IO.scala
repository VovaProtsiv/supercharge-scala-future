...
import exercises.action.fp.IO.{async, fail}
...
trait IO[A] {
  ...
  def retry(maxAttempt: Int): IO[A] = {
    if (maxAttempt <= 0) IO.fail(new IllegalArgumentException("Counter is too low"))
  ...
  def attempt: IO[Try[A]] =
    IO.async { callback =>
      unsafeRunAsync(tryA => callback(Success(tryA)))
    }
  ...
  def parZip[Other](other: IO[Other])(ec: ExecutionContext): IO[(A, Other)] =
    async { callback =>
      ...
      ec.execute(() => unsafeRunAsync(tryA => promise1.complete(tryA)))
      ec.execute(() => other.unsafeRunAsync(tryOther => promise2.complete(tryOther)))

      val eventualTuple: Future[(A, Other)] = promise1.future.zip(promise2.future)

      eventualTuple.onComplete(callback)(ec)
    }
...
object IO {
  ...
  def parSequence[A](actions: List[IO[A]])(ec: ExecutionContext): IO[List[A]] =
    actions.
      foldLeft(IO(List.empty[A]))(
        (state, action) => state.parZip(action)(ec).map {
          case (r1, r2) => r2 :: r1
        }).map(_.reverse)
