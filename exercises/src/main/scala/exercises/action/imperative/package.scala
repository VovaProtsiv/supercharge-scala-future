...
package object imperative {
  ...
  @tailrec
  private def retry[A](maxAttempt: Int, counter: Int)(action: => A): A = {
    require(maxAttempt > 0, "The maxAttempt must be greater then 0")
    Try(action) match {
      case Failure(_) if counter < maxAttempt => retry(maxAttempt, counter + 1)(action)
