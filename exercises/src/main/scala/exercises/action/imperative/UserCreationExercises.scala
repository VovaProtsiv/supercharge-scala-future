...
import exercises.action.imperative.UserCreationExercises.readUser
...
object UserCreationExercises {
  ...
  private val errorYesNoMsg = "Incorrect format, enter \"Y\" for Yes or \"N\" for \"No\""
  private val errorBirthMsg = """Incorrect format, for example enter "18-03-2001" for 18th of March 2001"""
  ...
  // else, `readSubscribeToMailingList` throws an exception.
  // For example,
  ...
  def parseYesNo(answer: String): Boolean = {
    answer match {
      case "Y" => true
      case "N" => false
      case _ => throw new IllegalArgumentException(errorYesNoMsg)
    }
  }
  ...
  def readUser(console: Console, clock: Clock): User = {
    ...
    val dateOfBirth = readDateOfBirthRetry(console,maxAttempt)
    val subscribedToMailingList = readSubscribeToMailingListRetry(console,maxAttempt)
    val creationAt = clock.now()
    val user = User(name, dateOfBirth, subscribedToMailingList, creationAt)
  ...
  def readSubscribeToMailingListRetry(console: Console, maxAttempt: Int): Boolean = {
    retry(maxAttempt){
      console.writeLine("Would you like to subscribe to our mailing list? [Y/N]")
      onError(parseYesNo(console.readLine()),_=>console.writeLine(errorYesNoMsg))
      }
  ...
  def readDateOfBirthRetry(console: Console, maxAttempt: Int): LocalDate = {
    retry(maxAttempt){
      onError(readDateOfBirth(console),_=>console.writeLine(errorBirthMsg))
    }
  }
  ...
  private def checkMaxAttempt(maxAttempt: Int): Unit = {
    require(maxAttempt > 0, "The maxAttempt must be greater then 0")
  }
}
